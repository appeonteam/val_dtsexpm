...
type p_1 from picture within w_table_upd
end type
...
event ue_enablepagerupd ( )
event ue_enablemgrupdate ( )
p_1 p_1
...
type variables
int currentTab
boolean allowUpdPager = false
boolean allowUpdMgr = false

int currPagerIdSize,currPagerIdMinSize
...
event ue_enablepagerupd();allowupdpager = true;
end event

event ue_enablemgrupdate();allowUpdMgr = true;
end event
...
st_today.text = string(today(),"mmmm dd, yyyy")
...
event resize;
cb_new.y    = This.WorkSpaceHeight() - cb_new.Height - 55
...
string picturename = "smartpaging_light_icon_32.jpg"
...
if currentTab >= 3 then
	if dw_1.GetItemString(dw_1.GetRow(),'dept') = 'DEFT' then
		MessageBox("Error","Can't delete the Default department!",StopSign!)
		return
	end if
end if
...
//	dw_1.SetItem ( dw_1.RowCount(), "pager_id", string(dw_1.RowCount(),"0000") )
	...
	if currentTab >= 3 then
		dw_1.SetItem ( dw_1.RowCount(), "zone_code", "Z0") // Z0 is All Zones
	end if
end if
