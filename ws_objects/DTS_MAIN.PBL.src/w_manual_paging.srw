...
type cb_exeit from picturebutton within w_manual_paging
end type
type cb_send from picturebutton within w_manual_paging
end type
type p_2 from picture within w_manual_paging
end type
...
public function string wf_strp_spcl_char (string instr)
end prototypes
...
public function string wf_strp_spcl_char (string instr);int si, i, ascv
si = len(inStr)
string nStr
nStr=''
i = 1
...
pg_msg = wf_strp_spcl_char(RightTrim(mle_page_msg.text))
...
do while lb_selected.TotalItems() >= inx
	...
	ass.call_number = ""
	ass.page_sent = 'N'
//	ass.special_msg = mle_page_msg.text
