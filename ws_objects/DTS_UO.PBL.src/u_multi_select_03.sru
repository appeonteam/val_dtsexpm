$PBExportHeader$u_multi_select_03.sru
forward
global type u_multi_select_03 from userobject
end type
type uo_ddlb_1 from dropdownlistbox within u_multi_select_03
end type
type uo_lb_1 from listbox within u_multi_select_03
end type
end forward
...
global u_multi_select_03 u_multi_select_03
...
nOpenHgt = uo_ddlb_1.Height + uo_ddlb_1.Y+300
...
string item[] = {"[All]","[Selected]","[PM-Preset]"}
...
MessageBox("rajan",this.text);
daywise_pm_selection=Trim(Upper(this.text));
MessageBox("Value saved in daywise_pm_selection",daywise_pm_selection);
MessageBox("the equipment number is ",equipment_number);


If Trim(Upper(this.text)) = Trim(Upper("[7-Day PMs]")) Then
	    MessageBox("Rajan","7 Days Selected");
		...
		MessageBox("loadlb",load_lb);
		if load_lb = "subtask" then
		 MessageBox("Rajan","7 Days Selected");	
		 cColumn2Retrieve="subproblem_desc";
		 cTable2Use="vzt_equipment_task_map,subproblem";
		 cDbWhere="  period=7 and equipment_id = '"+equipment_number+"' ";
		 cDbWhere = cDbWhere+" and vzt_equipment_task_map.subproblem_number=subproblem.subproblem_number";
		 
		  	MessageBox( "cColumn2Retrieve",cColumn2Retrieve  );
	   		MessageBox( "cTable2Use",cTable2Use  );
			MessageBox( "cDbWhere",cDbWhere  );
...
IF Upper( Trim( uo_ddlb_1.Text ) ) = "[7-Day PMs]" Then
	MessageBox("Selected PM","7 Day");	
end if
