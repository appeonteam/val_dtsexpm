$PBExportHeader$uo_qgen_tabpage_parms.sru
forward
global type uo_qgen_tabpage_parms from uo_tabpage_dw
end type
type dw_queries from uo_vzt_dwbase within uo_qgen_tabpage_parms
end type
type cb_saveq from commandbutton within uo_qgen_tabpage_parms
end type
type cb_delquery from commandbutton within uo_qgen_tabpage_parms
end type
type cb_newq from commandbutton within uo_qgen_tabpage_parms
end type
end forward
...
event ue_set_questionid ( string as_questionid )
dw_queries dw_queries
cb_saveq cb_saveq
cb_delquery cb_delquery
cb_newq cb_newq
end type
global uo_qgen_tabpage_parms uo_qgen_tabpage_parms

type variables
string is_report, is_questionid
boolean 	ib_qrefresh
end variables

event ue_set_questionid;//
is_questionid	= as_questionid
dw_queries.Retrieve( is_report, gnv_appman.is_userid, is_questionid)

end event

on uo_qgen_tabpage_parms.create
int iCurrent
call super::create
...
on uo_qgen_tabpage_parms.destroy
call super::destroy
...
type dw_1 from uo_tabpage_dw`dw_1 within uo_qgen_tabpage_parms
event ue_dateset ( )
integer y = 12
integer width = 2222
integer height = 992
...
Else
	...
	if long(is_questionid) > 0 and not ib_qrefresh then 
		dw_queries.Retrieve( is_report, gnv_appman.is_userid, is_questionid)
	End If 
END IF 
end event
...
string dataobject = "d_qgen_rpt_querylist"
...
If MessageBox('Confirm Delete', 'Are you sure you want to delete query "'+ls_qdescr+'" ?', Question!, YesNo!, 2) = 1 then
	DELETE FROM VZT_REPORTDEFPARMS WHERE 
	reportid = :ls_report AND 
	...
	dw_queries.Retrieve( ls_report, gnv_appman.is_userid, is_questionid)
END IF	
end event
