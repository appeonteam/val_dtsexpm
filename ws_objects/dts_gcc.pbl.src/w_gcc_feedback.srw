...
// web options
string 	is_url, is_propid, is_dparm, is_web_default 
boolean		ib_web_request, ib_webdb
transaction	i_web_tran
...
//// VZT WEB Copyright Protection DO NOT REMOVE! ------------------------
//UPDATE vzt_sysoptions  
//SET option10='' , option12='', option14 ='L'
//where id = 1;
//COMMIT;
//DELETE FROM vzt_html;
//COMMIT;
//DELETE FROM vzt_web_style;
//COMMIT;
//// VZT WEB Copyright Protection DO NOT REMOVE! ------------------------
...
event ue_web_check;Return -8
		

end event

event ue_web_letter;return 
...
this.Control[iCurrent+2]=this.gb_web
...
this.Control[iCurrent+9]=this.pb_task
this.Control[iCurrent+10]=this.pb_thankyou
this.Control[iCurrent+11]=this.pb_apology
this.Control[iCurrent+12]=this.pb_call
this.Control[iCurrent+13]=this.dw_gcc_history
this.Control[iCurrent+14]=this.dw_dupcheck
this.Control[iCurrent+15]=this.r_1
this.Control[iCurrent+16]=this.pb_rebates
this.Control[iCurrent+17]=this.pb_amends
...
this.Control[iCurrent+24]=this.dw_name_search
this.Control[iCurrent+25]=this.cb_reset
...
ELSE
	...
	If ll_rc > 0 and ll_row > 0 then 
		...
		if gcc_room_status = DataModified! or gcc_arr_status = DataModified! or gcc_dep_status	= DataModified! then
...
event dw_mainlist::doubleclicked;call super::doubleclicked;	if id_surveyid <= 0 or id_gcc <= 0 then 
	...
	ldt_ansdate = dw_data.GetItemDateTime( dw_data.GetRow() , 'gcc_ansdate') 
...
//	if dw_data.GetRow() > 0 then
			if POS( is_accessflags, 'A') <= 0 then // not admin 
				if DaysAfter ( date(ldt_ansdate), date(f_sysdate() )) > 1  or is_webfilled = 'Y' then
					ib_readonly = true 
				end if
			end if 
//	end if 
...
if wf_gcc_selected() < 0 then return  
SetPointer( Hourglass!)
OpenSheetWithParm( w_gcc_letter_apology, istr_parms, w_gcc_main, 0, Layered!)
end event
...
CHOOSE CASE ls_code
	CASE 'TYL', 'APL', 'REL', 'AML', 'FRL', 'PGL', 'WSR', 'WL' // 12.18.2001
		istr_parms.id = ls_id 
		OpenSheetWithParm( w_gcc_letter_view, istr_parms, w_gcc_main, 0, Layered!)
END CHOOSE
